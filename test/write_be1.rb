...
def write_be1( season )
  season = SportDb::Import::Season.new( season )  ## normalize season

  in_path = "../cache.leagues/o/#{season.path}/be.1.csv"
  matches = SportDb::CsvMatchParser.read( in_path )

  pp matches[0]
  puts "#{matches.size} matches"

  matches = normalize( matches, league: 'be.1' )

  ## split into ? stages into two files
  ## - Regular Season
  ...
  # out_dir = './tmp'
  out_dir = '../../../openfootball/world/europe/belgium'


  league_name = 'Belgian First Division A'


  buf = build_stage( stages['Regular Season'],
                     name: "#{league_name} #{season.key}, Regular Season" )
  puts buf
  write_buf( "#{out_dir}/#{season.path}/1-firstdivisiona-i.txt", buf )


  buf = String.new('')
  buf << build_stage( stages['Championship play-off'],
                      name: "#{league_name} #{season.key}, Championship play-off" )
  puts buf
  write_buf( "#{out_dir}/#{season.path}/1-firstdivisiona-ii.txt", buf )
end
